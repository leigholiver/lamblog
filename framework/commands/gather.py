...
class gather(command):
    def run(self, data):
        ...
        if hasher.has_changed(os.path.abspath(support_path)):
        ...
        # copy api src
        if hasher.has_changed(os.path.abspath(api_path)):
            os.system("cp -r " + api_path + "/* " + build_dir)
        else:
            print("No api update needed, skipping")        

        # install packages
        if hasher.has_changed(os.path.abspath(api_path + "/packages.txt")):
